...
export default function ExperienceAdd({ setExperience , setIsSet ,resetData}) {
  ...
  const submit = async ()=>{
    ...
    await Axios.post("/api/experience", form ) .then(res=>res).then(data=>console.log(data));
    setIsSet(prev=>!prev)
  }
  return (
    <div
    ...
    >
      ...
      >
        ...
        <div className="text-white w-full flex justify-center mb-5">
          ...
          <textarea name="description"     onChange={(e) => setFormFunc(e)} className="text-[15px] text-black rounded p-1 w-[250px] outline-none border-0  h-[100px] resize-none"></textarea>
        </div>
        <div className="mt-5">
